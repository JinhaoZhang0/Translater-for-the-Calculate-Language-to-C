...
and ast_ize_SL (sl:parse_tree) : ast_sl =
  ...
  | PT_nt ("SL", [stmt; stmt_list])
        -> (ast_ize_S stmt)::(ast_ize_SL stmt_list)
...
and ast_ize_S (s:parse_tree) : ast_s =
  ...
  | PT_nt ("S", [PT_term "if"; condition ; stmt_list; PT_term "end"]) 
        -> AST_if (ast_ize_C condition, ast_ize_SL stmt_list)
  | PT_nt ("S", [PT_term "while"; condition ; stmt_list ; PT_term "end"]) 
        -> AST_while (ast_ize_C condition, ast_ize_SL stmt_list)
...
let rec get_unused_vars (vl:ast_vars) (str:string) : string =
  match vl with
  | h::t -> if ((extract_varint h) = 0)
            then ((get_unused_vars (t)) (str ^ (extract_varstring h) ^ " "))
...
let rec translate (ast:ast_sl)
  ...
  let (vars, output_program) = translate_sl [] ast in
  let output_program = prologue ^ output_program ^ "}\n" in
    print_string output_program;
  get_unused_vars vars "", output_program


(*  commented out so this code will compile *)

and translate_sl (vars:ast_vars) (ast:ast_sl) = 
  match ast with
  | h::t -> let (new_vars, str1) = translate_s vars h in
            let (new_vars, str2) = translate_sl new_vars t in
              new_vars, str1 ^ str2 ^ "\n"
  | [] -> vars, ""

and translate_s (vars:ast_vars) (ast:ast_s) : ast_vars * string = 
  match ast with
  | AST_assign (id, expr)
      -> translate_assign vars id expr
  | AST_read (id)
      -> translate_read vars id
  | AST_write (expr)
      -> translate_write vars expr
  | AST_if (cond, stmt_list)
      -> translate_if vars cond stmt_list
  | AST_while (cond, stmt_list)
      -> translate_while vars cond stmt_list
  | _ -> vars, "translate error"

and translate_assign (vars:ast_vars) (id:string) (expr:ast_e) : ast_vars * string = 
  let new_var = [AST_var(id, 0)] in
  if List.mem_assoc new_var vars
  then (let (new_vars, str) = translate_expr vars expr in
    new_vars, id ^ " = " ^ str ^ ";\n")
  else (let (new_vars, str) = translate_expr vars expr in
    List.append (new_vars) ([AST_var(id, 0)]), "int " ^ id ^ " = " ^ str ^ ";\n")

and translate_read (vars:ast_vars) (id:string) : ast_vars * string = 
  List.append (vars) ([AST_var(id, 0)]), "int " ^ id ^ " = getint();\n"

and translate_write (vars:ast_vars) (expr:ast_e) : ast_vars * string = 
  let (new_vars, str) = translate_expr vars expr in
    new_vars, "putint(" ^ str ^ ");\n"

and translate_if (vars:ast_vars) (cond:ast_c) (stmt_list:ast_sl) : ast_vars * string = 
  let (new_vars, str1) = translate_cond vars cond in
  let (new_vars, str2) = translate_sl new_vars stmt_list in
    new_vars, "if " ^ str1 ^ " {\n" ^ str2 ^ "}\n"

and translate_while (vars:ast_vars) (cond:ast_c) (stmt_list:ast_sl) : ast_vars * string = 
  let (new_vars, str1) = translate_cond vars cond in
  let (new_vars, str2) = translate_sl new_vars stmt_list in
    new_vars, "while " ^ str1 ^ " {\n" ^ str2 ^ "}\n"

and translate_expr (vars:ast_vars) (expr:ast_e) : ast_vars * string = 
  match expr with
  | AST_binop ("/", lhs, rhs)
      -> let (new_vars, str1) = translate_expr vars lhs in
         let (new_vars, str2) = translate_expr new_vars rhs in
          new_vars, "divide(" ^ str1 ^ ", " ^ str2 ^ ")"
  | AST_binop (op, lhs, rhs)
      -> let (new_vars, str1) = translate_expr vars lhs in
         let (new_vars, str2) = translate_expr new_vars rhs in
          new_vars, "(" ^ str1 ^ op ^ str2 ^ ")"
  | AST_num (n) -> vars, n
  | AST_id (id) -> (List.append (vars) ([AST_var(id, 1)])), id

and translate_cond (vars:ast_vars) (cond:ast_c) : ast_vars * string = 
  match cond with
  | ("=", lhs, rhs)
      -> let (new_vars, str1) = translate_expr vars lhs in
         let (new_vars, str2) = translate_expr new_vars rhs in
